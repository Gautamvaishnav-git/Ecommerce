...
const Search: React.FC = () => {
  ...
  const { response, fetchErr, loading, FetchOnAction } = useFetch<IProduct[]>({
    url: `${baseURI}/filter/search/?q=${params.query}`,
  });
  useEffect(() => {
    FetchOnAction();
    response && setSearchList(response);
  }, [response, params]);
  ...
  return (
    <>
      ...
      {searchList?.length === 0 && (
        <div className="container mx-auto text-center text-2xl text-indigo-500">
          No Item Found!!
        </div>
      )}
